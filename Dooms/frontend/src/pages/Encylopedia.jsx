...
export const Encyclopedia = () => {
  ...
  const closeModal = () => setSelectedSpecies(null);

  return (
    <div className="min-h-screen bg-white py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-12">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-primary mb-4">Marine Encyclopedia</h1>
          <p className="text-base sm:text-lg text-gray-600 max-w-2xl mx-auto mb-8">
            Explore our comprehensive database of marine species and learn about ocean life
          </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredSpecies.map((species) => (
            <article
              ...
              className="bg-white shadow-lg rounded-2xl overflow-hidden border border-gray-100 hover:shadow-2xl hover:scale-105 transition-all duration-300 cursor-pointer group"
            >
              <div className="relative overflow-hidden">
                ...
                <div className="absolute top-3 right-3">
                  <span className="inline-block bg-white/90 backdrop-blur-sm text-accent text-xs sm:text-sm px-3 py-1 rounded-full font-medium shadow-lg">
                    {species.conservationStatus}
                  </span>
                </div>
              </div>
              <div className="p-4 sm:p-6">
                <h2 className="text-lg sm:text-xl font-bold text-primary group-hover:text-accent transition-colors">{species.name}</h2>
                <p className="text-xs sm:text-sm text-accent italic mt-1">{species.scientificName}</p>
                <p className="mt-3 text-sm sm:text-base text-gray-700 line-clamp-3">{species.description}</p>
                
                <div className="mt-4 flex items-center justify-between">
                  <span className="text-xs sm:text-sm text-primary font-medium">Click to learn more</span>
        ...
        {filteredSpecies.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 text-lg">No species found matching "{searchTerm}"</p>
          </div>
        )}
      </div>

      {/* Modal for Full Species Details */}
      {selectedSpecies && (
        <div 
        ...
        >
          ...
          >
            ...
            <div className="relative h-64 sm:h-80 overflow-hidden rounded-t-3xl">
              ...
              <button
                onClick={closeModal}
                className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm text-gray-800 rounded-full w-10 h-10 flex items-center justify-center hover:bg-white transition-all shadow-lg text-xl font-bold"
              >
                ✕
              </button>
              <div className="absolute bottom-6 left-6 right-6 text-white">
                <h2 className="text-3xl sm:text-4xl font-bold mb-2">{selectedSpecies.name}</h2>
                <p className="text-lg sm:text-xl italic opacity-90">{selectedSpecies.scientificName}</p>
              </div>
            </div>
            ...
            <div className="p-6 sm:p-8 space-y-6">
              ...
              <div className="flex items-center gap-3 flex-wrap">
                <span className={`inline-block px-4 py-2 rounded-full text-sm font-semibold ${
                  selectedSpecies.conservationStatus === 'Critically Endangered' ? 'bg-red-100 text-red-800' :
                  selectedSpecies.conservationStatus === 'Endangered' ? 'bg-orange-100 text-orange-800' :
                  selectedSpecies.conservationStatus === 'Vulnerable' ? 'bg-yellow-100 text-yellow-800' :
                  selectedSpecies.conservationStatus === 'Near Threatened' ? 'bg-blue-100 text-blue-800' :
              ...
              <div className="bg-accent/10 border-l-4 border-accent p-4 rounded-r-xl">
                ...
                <p className="text-gray-800 italic">{selectedSpecies.interestingFact}</p>
              </div>

              {/* Habitat */}
              <div>
                <h3 className="text-xl font-bold text-primary mb-3">🏝️ Habitat</h3>
                <p className="text-gray-700 leading-relaxed">{selectedSpecies.habitat}</p>
              </div>

              {/* Diet */}
              <div>
                <h3 className="text-xl font-bold text-primary mb-3">🍽️ Diet</h3>
                <p className="text-gray-700 leading-relaxed">{selectedSpecies.diet}</p>
              </div>

              {/* Ecology */}
              <div>
                <h3 className="text-xl font-bold text-primary mb-3">🌊 Ecology</h3>
                <p className="text-gray-700 leading-relaxed">{selectedSpecies.ecology}</p>
              </div>

              {/* Behavior */}
              <div>
                <h3 className="text-xl font-bold text-primary mb-3">🐾 Behavior</h3>
                <p className="text-gray-700 leading-relaxed">{selectedSpecies.behavior}</p>
              </div>

              {/* Fun Facts */}
              {selectedSpecies.funFacts && selectedSpecies.funFacts.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-primary mb-3">✨ Fun Facts</h3>
              ...
              {/* Optimal Conditions */}
              {selectedSpecies.optimalConditions && (
                <div>
                  <h3 className="text-xl font-bold text-primary mb-3">📊 Optimal Conditions</h3>
                  <div className="grid sm:grid-cols-3 gap-4">
                    <div className="bg-blue-50 p-4 rounded-xl">
                      <p className="text-sm font-semibold text-primary mb-1">Temperature</p>
                      <p className="text-2xl font-bold text-blue-600">
                        {selectedSpecies.optimalConditions.temperature.min}°C - {selectedSpecies.optimalConditions.temperature.max}°C
                      </p>
                    </div>
                    <div className="bg-cyan-50 p-4 rounded-xl">
                      <p className="text-sm font-semibold text-primary mb-1">Salinity</p>
                      <p className="text-2xl font-bold text-cyan-600">
                        {selectedSpecies.optimalConditions.salinity.min} - {selectedSpecies.optimalConditions.salinity.max} ppt
                      </p>
                    </div>
                    <div className="bg-indigo-50 p-4 rounded-xl">
                      <p className="text-sm font-semibold text-primary mb-1">Depth</p>
                      <p className="text-2xl font-bold text-indigo-600">
                        {selectedSpecies.optimalConditions.depth.min}m - {selectedSpecies.optimalConditions.depth.max}m
                      </p>
                    </div>
                  </div>
                </div>
              ...
              {selectedSpecies.threats && selectedSpecies.threats.length > 0 && (
                <div>
                  <h3 className="text-xl font-bold text-primary mb-3">⚠️ Threats</h3>
                  <div className="grid sm:grid-cols-2 gap-2">
                    {selectedSpecies.threats.map((threat, index) => (
                      <div key={index} className="flex items-center gap-2 bg-red-50 px-4 py-2 rounded-lg">
